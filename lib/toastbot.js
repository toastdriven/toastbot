...
Toastbot.prototype.setup = method(function(self) {
  ...
  self.client.on('join'+self.channel, function(nick) {
    self.log(nick+' joined '+self.channel);
  });
  self.client.on('part'+self.channel, function(nick) {
    self.log(nick+' left '+self.channel);
  });
...
Toastbot.prototype.fatpita = handler(method(function(self, nick, text) {
  var text = self.is_direct_command('fatpita', text);
  ...
  var req = http.get(options, function(res) {
    ...
    self.log("Failed to load random fatpita image: "+e.message);
  });
  
  return true;
}));
Toastbot.prototype.fatpita.__doc__ = "Get a random fatpita image. For the lulz.";
